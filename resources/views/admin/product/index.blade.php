                                ...
                                <tr class="border-b border-gray-200">
                                    ...
                                    <td class="px-6 py-4 text-gray-700">
                                        <img src="{{ asset('uploads/products/' . $product->img) }}"
                                            alt="{{ $product->name_product }}" class="w-16 h-16 object-cover">
                                    </td>
                                    <td class="px-6 py-4 text-gray-700">
                                        @if ($product->kategori_produk && $product->kategori_produk->count())
                                            @foreach ($product->kategori_produk as $kategori)
                                                <span
                                                    class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-md mr-2">{{ $kategori->name_kategori }}</span>
                                            @endforeach
                                        @else
                                    ...
                                    <td class="px-6 py-4 text-gray-500">
                                        <a href="{{ route('admin.product.edit', $product->id) }}"
                                            class="text-indigo-600 hover:text-indigo-800 mr-4">Edit</a>
